...
suite.addBatch "When we set up two apps":
  ...
  "and we register one user on each":
    topic: ->
      callback = @callback
      Step (->
        newCredentials "collector", "give*me*that", "social.localhost", 80, @parallel()
        newCredentials "friedlander", "shadowed*1", "photo.localhost", 80, @parallel()
        return
      ), callback
      return
    ...
    "and one user follows the other":
      topic: (cred1, cred2) ->
        url = "http://social.localhost/api/user/collector/feed"
        act =
          verb: "follow"
          object:
            id: "acct:friedlander@photo.localhost"
            objectType: "person"
      ...
      "and we wait a few seconds for delivery":
        ...
        "and the second user posts an image publicly":
          topic: (act, cred1, cred2) ->
            url = "http://photo.localhost/api/user/friedlander/feed"
            callback = @callback
            post =
              verb: "post"
              to: [
          ...
          "and we wait a few seconds for delivery":
            ...
            "and we check the first user's inbox":
              topic: (posted, followed, cred1, cred2) ->
                callback = @callback
                url = "http://social.localhost/api/user/collector/inbox"
