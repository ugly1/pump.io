...
Client::asActivityObject = (callback) ->
  ...
  props.displayName = client.title  if client.title
  props.content = client.description  if client.description
  if client.webfinger
    ...
    props.id = "urn:uuid:" + uuidv5(client.consumer_key)  if _.isString(client.consumer_key)
    switch client.type
      when "web"
        props.objectType = ActivityObject.SERVICE
      when "native"
